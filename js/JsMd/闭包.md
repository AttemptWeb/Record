...
```
```函数func```也就不写出来了。不是特别了解作用域链的可以查看[JavaScript深入之执行上下文](https://github.com/mqyqingfeng/Blog/issues/8)，，```作用域链中会保存局部变量、全局变量、函数参数```。在正常函数调用后，作用域链被销毁，但是当存在闭包时，作用域链会被保存。
